...
export function ClusterDetail({ cluster }: { cluster: GraphCluster }) {
  return (
    <div className="rounded-2xl border border-stone-100 bg-white p-6 shadow-sm">
      <div className="mb-5 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <div>
          <h3 className="text-sm font-semibold text-stone-900">
            {cluster.members.length} codes, one material
          </h3>
          <p className="text-xs text-stone-400">
            Across {cluster.orgs.length} CPSEs. None of them could see the others.
          </p>
        </div>
        <span className="font-mono text-[11px] text-stone-400">{cluster.national_code}</span>
      </div>

      {/* The messy side, in full. Each line is what one organisation actually wrote. */}
      <ul className="mb-1 flex flex-col divide-y divide-stone-100">
        {cluster.members.map((m) => (
          <li key={m.record_id} className="flex items-start gap-3 py-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-mono text-[12px] leading-snug text-stone-700">
                {m.raw_description}
              </p>
      ...
      <div className="mt-4 rounded-xl bg-stone-900 px-4 py-3">
        <p className="font-mono text-[10px] uppercase tracking-widest text-stone-400">
          resolves to
        </p>
        <p className="mt-1 font-mono text-[13px] text-white">{cluster.standardised_short}</p>
        <p className="mt-0.5 font-mono text-[11px] text-stone-400">{cluster.national_code}</p>
      </div>
        ...
        <div className="mt-4 border-t border-stone-100 pt-3">
          ...
          {cluster.alternatives.map((a) => (
            <div key={a.record_id} className="mb-2">
              <p className="font-mono text-[11px] text-stone-600">{a.raw_description}</p>
              {a.condition && (
                <p className="mt-0.5 text-[11px] leading-relaxed text-stone-400">{a.condition}</p>
              )}
            </div>
          ))}
        </div>
